%function cluster_tetrode_multexp
% Matlab codes for reading from TTank

clear all;
pack
close all
Tank_Name='071107_wt_tet'
nblocks = 6;
...
Block_Name = {'drift1' 'wn1' 'sb1'  'cp_tf08' 'flashgrat_08'  'bars16d1' 'bars16d1b'}

%Block_Name = {'drift1'  'bars16d1'}
...
max_range = 14:22;
   ...
   pack
    ...
    dlmwrite(fname,score(:,pca_use),'-append','delimiter',' ');

    dos('C:\KlustaKwik pca 1 -MinClusters 5 -MaxClusters 10 -MaxPossibleClusters 13 -Verbose 0 -nStarts 3');
    %n_pca=4;
    cl_name = 'pca.clu.1';
...
%      plot(1:size(mean_wvform,1),mean_wvform (:,(tet-1)*4+1 : (tet-1)*4+4,c)');
     ...
     min_t = min(mean_wvform (:,(tet-1)*4+1 : (tet-1)*4+4,c),[],1);
