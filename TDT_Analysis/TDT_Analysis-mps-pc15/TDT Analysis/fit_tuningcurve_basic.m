function [theta_pref OSI A1 A2 w B null_rate yfit] = fit_tuningcurve_basic(R, theta_ind);

% temp_baseline =min(R); 
% R=R-temp_baseline;  %%%need to keep things positive for fourier analysis
theta_ind
if abs(min(R))>max(R) & abs(min(R))>2 %%% check for units that are inhibited more than excited
    R = -R;
    inverted=1;
else
    inverted=0;
