%function drift_orientfreq_cluster
...
for cell_n = 1:size(cells,1)
  ...
  time2=max(times);
   ...
   if blank_stim
       spont_avg = R(cell_n,n_rows*n_col)
   else
 ...
 orientfreq = reshape(R(cell_n,1:n_col*n_rows),n_rows,n_col)'-spont_avg;
 orient_tuning_all = mean(orientfreq,2);
 figure
 plot(orient_tuning_all);
 [max_resp pref_orient] = max(orient_tuning_all);
 freq_tuning = orientfreq(pref_orient,:);
 figure
 plot(freq_tuning);
 [max_resp pref_freq] = max(freq_tuning);
 orient_tuning = orientfreq(:,pref_freq);
 figure
 plot(orient_tuning);
 f1f0 = F1(cell_n,(pref_orient-1)*n_col +pref_freq)/ F0(cell_n,(pref_orient-1)*n_col +pref_freq)
