...
clear Epocs trial_no Epocs_TS N Spike_TS index TS_xTrg Spike_Timing;
  ...
  i=orientation;
    ...
    n_eps(orientation) = size(eps,2);
    all_eps(orientation,:,1:n_eps(orientation)) = eps;
    clear phi
    temp_filt = ones(1,20);
    ...
    figure(spec)
       ...
       axis off
         set(gca,'XTickLabel',[])
        set(gca,'YTickLabel',[])
        all_power(orientation,1:size(pwr_short,2),1) = mean(pwr_short(1:(10/df),:),1);
        all_power(orientation,1:size(pwr_short,2),2) = mean(pwr_short((50/df):(70/df),:),1);        
end
...
for cell_n = 1:size(cells,1)
   ...
   for orientation =orient_list;
        ...
        index = zeros(1,N);
        TS_xTrg=index;
        for i = 1:n_eps(orientation+1);
            epochSpikes = find(Spike_TS>=all_eps(orientation+1,2,i) & Spike_TS<all_eps(orientation+1,3,i));
            index(epochSpikes)=i;
            TS_xTrg(epochSpikes)=all_eps(orientation+1,2,i);
        end
        ...
        numtrials = trial_no
         ...
         %%%LFP specific
        used_index= find(all_power(orientation+1,:,2)./all_power(orientation+1,:,1)<1.5);
        lfp_selection = 'logamma'
        for i = 1:trial_no;
            if ~isempty(find(used_index==i))
                new_index(i)=find(used_index==i);
            end
        end
         numtrials = length(used_index);
         trial_no = length(used_index); 
        ...
        %%% LFP specific
        if ~isempty(Spike_Timing)
            Spike_Timing = Spike_Timing(ismember(index,used_index));
            index = index(ismember(index,used_index));
    ...
    saveas(rast_fig,fullfile(pname,sprintf('bar_rast_nb%s_%d_%d%s',Block_Name,channel_no,clust_no,lfp_selection)),'fig');
    saveas(hist_fig,fullfile(pname,sprintf('bar_hist_nb%s_%d_%d%s',Block_Name,channel_no,clust_no,lfp_selection)),'fig');
