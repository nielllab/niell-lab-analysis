...
cells = [5 1; 9 2; 9 3; 9 4; 13 1 ; 13 3 ; 13 4]
cells = [  1 4]
...
%     ylabel('Hz');
    ...
    rf = rf_data';
     thresh = 1.5* spontmean(cell_n) + 0.1*(max(max(rf_data))-1.5*spontmean(cell_n))  %%% do this more statistically later, like 1std
     rfthresh = rf - thresh;
...
%     title('rf_data')
    ...
    for ax=1:2
        ...
        dispersion(cell_n,ax) = max(sqrt((nsx2/n) - (nsx/n)^2) , 0.5)
            
    end 
    
    f = fspecial('gaussian',3,.4);
    f = f/f(2,2);
    rf = imfilter(rf,f);
   ...
   theta = -15 * (pi/180);
    ...
    x(:,2)= sin(theta)*xgrid(:) + cos(theta)*ygrid(:);
      figure
   imagesc(reshape(x(:,1),[size(rf,1) size(rf,2)]));
   title('rf fit');
    
    figure
   imagesc(reshape(x(:,2),[size(rf,1) size(rf,2)]));
   title('rf fit');
   ...
   prior_est = rf2fit(prior_coeff,x);
        figure
   imagesc(reshape(prior_est,[size(rf,1) size(rf,2)]));
   title('prior');
   
   
      figure
   imagesc(reshape(obs,[size(rf,1) size(rf,2)]));
   title('obs');
