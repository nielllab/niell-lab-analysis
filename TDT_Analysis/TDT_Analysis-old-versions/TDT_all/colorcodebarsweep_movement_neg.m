%function barsweep_movement_neg
...
clear bars_theta bars_OSI bars_A1 bars_A2 bars_w bars_B
   ...
   for rep =1:1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if orientation<16
           ...
           hold on
            for i =1:length(trial_velocity);
        if trial_velocity(i)>0.6
            plot([0 3],[i i],'Color',[0.5 1 0.5],'LineWidth',6);
        else
           plot([0 3],[i i],'Color',[0.5 0.5 1],'LineWidth',6);
        end
 end
            ...
            plot ([Spike_Timing; Spike_Timing], [index-0.25;index+0.25], 'k','LineWidth',2);
            set(gca,'XTickLabel')
            set(gca,'YTickLabel',[])
...
%     title_text = sprintf('channel %d cluster %d',channel_no, clust_no);
    ...
    [bars_theta(cell_n,rep) bars_OSI(cell_n,rep) bars_A1(cell_n,rep) bars_A2(cell_n,rep) bars_w(cell_n,rep) bars_B(cell_n,rep) bars_null(cell_n,rep) y] = ...
    ...
    %%% to average width
    cond_peak = round(bars_theta(cell_n,rep)/(pi/8))+1
    if cond_peak>16
        cond_peak = 1;
    end
    ...
    rf_width(cell_n,rep) = sum(width(cell_n,conds).*amp(cell_n,conds))./sum(amp(cell_n,conds))
...
%     %plot(y,'g');
 ...
 tuningfig=figure
 plot(theta_ind*180/pi,squeeze(bartuning(cell_n,1,:))+barspont(cell_n,1));
 hold on
 plot(theta_ind*180/pi,squeeze(bartuning(cell_n,2,:))+barspont(cell_n,2),'g');
 plot(theta_ind*180/pi,ones(length(amp))*barspont(cell_n,1),'b:');
 plot(theta_ind*180/pi,ones(length(amp))*barspont(cell_n,2),'g:');
 ...
 figure
 plot(theta_ind*180/pi,squeeze(bartuning(cell_n,1,:).*barwidth(cell_n,1,:)));
 hold on
 plot(theta_ind*180/pi,squeeze(bartuning(cell_n,2,:).*barwidth(cell_n,2,:)),'g');
  title_text = sprintf('area channel %d cluster %d',channel_no, clust_no);
  
  figure
  plot(squeeze(bars_R(cell_n,1,1:16)));
  hold on
  plot(squeeze(bars_R(cell_n,2,1:16)),'g');
end  %%% cell
...
if use_afile
    ...
    save(afile_bar, 'bars_R','bartuning', 'barwidth', 'barspont', 'bars_theta', 'bars_A1', 'bars_A2', 'bars_w', 'bars_B','bars_null', 'rf_width','bars_dsi','-append');
end
...
% save(fullfile(apname,sprintf('analysis%s_%s',Tank_Name,Block_Name)));
