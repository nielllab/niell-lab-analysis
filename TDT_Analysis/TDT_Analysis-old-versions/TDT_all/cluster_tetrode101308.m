...
ch_map = 1:16;   %%% change this to do software remapping of sites
...
for tet=1:4
    ...
    %%% check for missing snippets on one channel, and realign
    ...
    clear Xshift allXdata chandata Y c_score
    pack
    ...
    clear pc_data pca_use I mn
    pack
    figure(ica_fig)
    if choose_pca
        ginput(1)   %%% just to bring ica_fig to foreground
    ...
    dos('C:\KlustaKwik pca 1 -MinClusters 5 -MaxClusters 10 -MaxPossibleClusters 12 -Verbose 0 -nStarts 2');
    %n_pca=4;
    cl_name = 'pca.clu.1';
    idx = uint8(dlmread(cl_name));
    n_clust = idx(1)
   
     %%% first cluster is outliers - uncomment this to  delete outlier cluster
    %     if n_clust>1    
    %         n_clust=n_clust-1;
    %         idx = idx-1;
    %     end
    idx = idx(2:size(idx,1));   %%% first
    for t = 1:4
 ...
 %%% plot 5 random waveforms from each cluster
    snipfig(tet)=figure;
    %%% colorcode 
    ...
    %%% plot histogram of spike amplitudes
    clear hist_data;
    ...
    %%% scatterplot of ICAs for each cluster
    clustfig(tet)=figure;
    ...
    for c = 1:n_clust %%% waveform parameters
        ...
        if trough_t+peak_t<size(mean_wvform,1)
            ...
            %%% fit parabola through three points to find peak
...
clear X Xica Xold  used nonc_score s score c_score
