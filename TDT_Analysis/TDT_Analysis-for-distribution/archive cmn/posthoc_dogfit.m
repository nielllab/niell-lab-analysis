...
for c =1:size(cells,1);
    ...
    data = drift_sftuning(c,[1 5 9 13 17 21 25]);
...
%     plot(fit_sfs,interpdata,'g');
    ...
    alldata(c,1) = w_pref_dog;
    alldata(c,2) = w_bw_dog;
    alldata(c,3) = peak_value;
    alldata(c,4) = cells(c,1);
    alldata(c,5) = cells(c,2);
    
end
alldata
xlswrite('sf_dog',alldata(:,1:5));
 %#ok<NOPRT>