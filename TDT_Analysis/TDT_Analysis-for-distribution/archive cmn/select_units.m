...
for tet=1:4
    ...
    select_fig = figure;
    set(gcf,'Position',[600 500 500 400]);
    for i =1:12;
        subplot(3,4,i);
...
pause
 ...
 pause
    ...
    used = find(goodcells);
        ...
        for i = 1:length(used)
        cluster_times{ncells+i} = squeeze(event_times_all(tet_ch,find(idx_all(tet_ch,:) == used(i))));
        end
...
save(fullaname, 'clusterfilename', 'Tank_Name',  'cells', 'nspikes', 'L_ratio', 'trough_depth', 'peak_height', 'trough_width', 'trough2peak','idx_all', 'pname','wv','cluster_times');
