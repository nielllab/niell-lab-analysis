...
if correct_spectrum
    ...
    inv_spectrum = (1./mean_spectrum).*(mean_spectrum>.1);
    figure
...
%     saveas(gcf,fullfile(pname,sprintf('hist%s_%d_%d',Block_Name,channel_no,clust_no)),'fig');
   ...
   %%%spike triggered variance (not covariance!)

    [x y] = meshgrid(-29:30,-29:30);
    sf =.06
    test_sta = sin(2*pi*x*sf).*exp(-(x.^2 + (y/2.5).^2)/(2*4^2));
   figure
   imagesc(test_sta')
    
  % test_sta=ones(size(test_sta));
   ...
   spike_p = 3*10^-2;
   for t= 1:n_frames;
       spike_gen =sum(sum(spike_p.*test_sta.*moviedata(:,:,t)));
       spike_gen(spike_gen<0)=0;
        n_spikes(t)  = poissrnd(spike_gen);
    end
    n_spikes=n_spikes.*(n_spikes>0);
        n_spikes=n_spikes;
       % n_spikes = round(n_spikes/(sum(n_spikes/n_total)));
    sum(n_spikes)
    ...
    for t = 1:10;
        spectrum =  fft2(single(sta_t(:,:,t))-128);
        sta_t(:,:,t) = ifft2(spectrum.*inv_spectrum)+128;
    end
    figure
    imagesc(sta_t(:,:,1)'-movavg',color_range );
    
    movie_spectrum =  fft2(movavg-128);
    movie_mean = ifft2(movie_spectrum.*inv_spectrum)+128;
    
      figure
    imagesc(sta_t(:,:,1)'-movie_mean',color_range );
    
    figure
    imagesc(fftshift(abs(fft2(sta_t(:,:,1)-mean(mean(sta_t(:,:,1)))))));    
...
%         end
    ...
    if correct_spectrum
        saveas(gcf,fullfile(pname,sprintf('sta_correct_nocont_%s_%d_%d',Block_Name,channel_no,clust_no)),'fig');
    else
