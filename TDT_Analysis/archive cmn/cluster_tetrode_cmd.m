function cluster_tetrode_cmd(Tank_Name,Block_Name, select_ica, subtract_mean, n_ica, max_snips, bname, output_path, disp_figs)
...
% modified 1/11 to reduce memory load and user interaction, and allow
...
if nargin==0
    ...
    disp_figs = input('display figures ? [y]/n ','s');
    if isempty(disp_figs)
        disp_figs='y';
    end
end
...
%     hist(I);
    ...
    if disp_figs=='n' & nargin>0
        fname = fullfile(output_path,sprintf('hist%s_%s_t%d',Tank_Name,bname,tet));
        saveas(histfig(tet),fname,'fig');
        fname = fullfile(output_path,sprintf('snip%s_%s_t%d',Tank_Name,bname,tet));
        saveas(snipfig(tet),fname,'fig');
        fname = fullfile(output_path,sprintf('clust%s_%s_t%d',Tank_Name,bname,tet));
        saveas(clustfig(tet),fname,'fig');
        close(histfig(tet))
        close(snipfig(tet)) 
        close(clustfig(tet))
    end
