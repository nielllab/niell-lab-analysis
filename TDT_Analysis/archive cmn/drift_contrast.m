...
%%% reshape, and use average across SF to determine preferred angle
    ...
    n_w = n_col;
     ...
     theta_resample = theta_ind+theta_pref;
     theta_resample = mod(theta_resample,2*pi);
     orientfreq(n_rows+1,:) = orientfreq(1,:);
     theta_ind(length(theta_ind)+1)=2*pi;
     
     for j = 1:n_col
         
         tuning_centered(:,j,cell_n) = interp1(theta_ind,orientfreq(:,j),theta_resample,'spline');
         
     end
     
     figure
     plot(tuning_centered(:,:,cell_n));
%      
%      
%      contrast_response = mean(orientfreq,1);
%      figure
% %     plot([0 contrasts],[0 contrast_response/max(contrast_response)]);
%      axis([0 1 0 1])
%      title(sprintf('unit %d %d',channel_no, clust_no));
%      xlabel('contrast')
%      ylabel('response')
%      
%     p = polyfit(contrasts,contrast_response/max(contrast_response),2);
%     figure
%      hold on
%     plot(contrasts,contrast_response/max(contrast_response)); 
%     plot(0:0.1:1,polyval(p,double(0:0.1:1)),'go');
   
%     peakresp_drift(:,cell_n) = max(contrast_response);
%     contrast_drift(:,cell_n) = contrasts;
%     response_drift(:,cell_n)= contrast_response;
%     p(3) = p(3)-0.5;
%     r = roots(p);
%     realroots = find(real(r)==r & r>0);
%    if isempty(realroots)
%        halfcontrast_drift(cell_n)=NaN;
%    else
%     halfcontrast_drift(cell_n) = min(r(realroots));
%      plot(halfcontrast_drift(cell_n),0.5,'r*')
%    end 
...
if use_afile
    contrast_tuning_centered = tuning_centered;

    
    save(afile, 'contrast_tuning_centered', '-append');
end
