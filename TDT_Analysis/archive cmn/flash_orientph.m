...
if full_field
 ...
 if ~exist('flash_on');
    flash_on = zeros(size(cells,1),1);
    flash_off = flash_on;
    flash_phasedependence=flash_on;
    flash_used = flash_on;
    flash_amp = flash_on;
    on_off_phasedependence= flash_on;
 end
...
for cell_n = 1:size(cells,1)
    ...
    for cond =0:n_cond-1
      ...
      if cond<n_rows*n_col;
       ...
       %% histograms
        ...
        R_mid(cell_n,cond+1) = sum(Spike_Timing >stim_onset & Spike_Timing<=stim_duration)/(stim_duration*trial_no);
        R_on(cell_n,cond+1) = sum(Spike_Timing<=stim_onset)/(stim_onset*trial_no);
        R_off(cell_n,cond+1) = sum(Spike_Timing> stim_duration & Spike_Timing<stim_offset)/((stim_offset-stim_duration)*trial_no);
    end
 ...
 figure(hist_fig);
 ginput(1);
 use = input('use data ? 0/1  : ');
if use && printfig
    print(hist_fig);
end
    n_orient = n_rows;
    orientfreq(:,1:n_col) = reshape(R_mid(cell_n,1:n_col*n_rows),n_col,n_rows)'-spont_avg;
    on_orientfreq(:,1:n_col) = reshape(R_on(cell_n,1:n_col*n_rows),n_col,n_rows)'-spont_avg;
    off_orientfreq(:,1:n_col) = reshape(R_off(cell_n,1:n_col*n_rows),n_col,n_rows)'-spont_avg;

    n_w = n_col;
    orientfreq_all = orientfreq+on_orientfreq+off_orientfreq;
    
     orient_tuning_mn = mean(orientfreq_all,2)';
    [amp, pref_ind] = max(orient_tuning_mn);
    pref_ind
    
    theta_ind = orients*pi/180;
    [flash_thetapref(cell_n) OSI flash_A1(cell_n) A2 flash_w(cell_n) flash_B(cell_n) flash_null(cell_n) yfit] ...
            = fit_tuningcurve(orient_tuning_mn, theta_ind);

    ph_tuning_curve = orientfreq_all(pref_ind,:);
    
   if use
        flash_amp(cell_n)= max(ph_tuning_curve);
        flash_phasedependence(cell_n) =abs(sum(ph_tuning_curve.*exp(sqrt(-1)*(0:pi/2:3*pi/2))))/sum(abs(ph_tuning_curve));
     
        [on_rate on_ind] = max(on_orientfreq(pref_ind,:))
        flash_control = max(orientfreq(pref_ind, :)) 
        flash_on(cell_n) = (on_rate-flash_control)/(on_rate+flash_control);

        [off_rate off_ind]= max(off_orientfreq(pref_ind,:))
        flash_off(cell_n) = (off_rate-flash_control)/(off_rate+flash_control);

        on_off_ph_tuning = on_orientfreq(pref_ind,:) - off_orientfreq(pref_ind,:);
        on_off_phasedependence(cell_n) = abs(sum(on_off_ph_tuning.*exp(sqrt(-1)*(0:pi/2:3*pi/2))))/sum(abs(on_off_ph_tuning));
...
if use_afile
    flash_orientfreq_all = orientfreq;

    flash_spont = R(:,n_cond);

    save(afile, 'flash_orientfreq_all', 'flash_used', 'flash_on','flash_off','flash_amp','flash_thetapref', 'flash_phasedependence', ...
        'on_off_phasedependence','flash_A1', 'flash_w','flash_null','flash_B', 'flash_spont', '-append');
end
flash_used
flash_amp
flash_on
flash_off
flash_phasedependence
on_off_phasedependence
