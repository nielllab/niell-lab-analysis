...
hma=hist(d_mnEyeAll(appAll==1 & stationary==0),nBins);
plot(nBins,hma/sum(stationary==0&appAll==1)); hold on; title('mn Eye Yaw, moving approach'); axis square
h=hist(d_mnEyeAll(appAll==1),nBins);
plot(nBins,h/sum(appAll==1)); hold on; title('mn Eye Yaw, approach'); axis square
legend('stationary','moving & approach','all approach');
plot([-3,-3],[.3,0],'k--');
plot([3,3],[.3,0],'k--');
