...
for c=0:1
    for vid=1:length(useData)
     clear useN tilt
        ...
        nBins=-90:5:90
        if sum(useN)>1
            tiltHist = (hist(tilt(useN),nBins))./sum(find(~isnan(tilt(useN)))& find(useN)');
        else
            tiltHist=NaN;
        end
...
tiltCounts(:,2)=nansum(tiltFull(:,:,2),1)./(length(useData))
...
%         plot(find(app),ones(sum(app),1)*90,'g.'); %ylim([-180 180])
        ...
        idx2=cluster(gm,newData);
         ...
         propNCVid(i,1) = length(find(~app' & idx2==clust))./(length(newData(~app)));
         propNCVid(i,2) = length(find(app' & idx2==clust))./(length(newData(app)));
         propCVid(i,1)=  length(find(~app' & idx2~=clust))./(length(newData(~app)));
         propCVid(i,2)= length(find(app' & idx2~=clust))./(length(newData(app)));
