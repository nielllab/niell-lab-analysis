...
%     set(gcf,'Name', sprintf('acc ch = %d',ch));
%     
%     for vid = 1:length(useFilt)
%          subplot(2,4,vid);
%         plot((accelData{vid}(:,ch))); hold on; axis square
%         if ch==1
%             plot((phiR{useFilt(vid)}-phiL{useFilt(vid)}));
%         elseif ch==2
%             plot((thetaR{useFilt(vid)}-thetaL{useFilt(vid)}));
%         elseif ch==6
%             plot(dTheta{useFilt(vid)});
%         end
%     end
% end
%%
...
afilename=sprintf('%s',ani,'Analyzed_112619_allSessions','.mat')
