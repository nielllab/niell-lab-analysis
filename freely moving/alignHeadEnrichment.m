function data = alignHeadEnrichment(fname, nPts,showMovies,psfilename, mousethresh, crickthresh)
...
if showMovies
    for t= 1:nframes;
        if ~isnan(centroid(1,t))
            ...
            hold off
            drawHeadEnrichment(centered(:,:,t)); axis square; axis([-70 70 -70 70])
            drawnow
        end
    end
end
...
drawHeadEnrichment(meanHead); axis square; axis equal
...
if showMovies
    ...
    for t= 1:nframes;
        if ~isnan(centroid(1,t))
            hold off
            drawHeadEnrichment(meanHead)
            drawHeadEnrichment(aligned(:,:,t)); axis square; axis([-70 70 -70 70])
            drawnow
        end
    end
end
...
drawHeadEnrichment(meanHeadAll); axis square; axis equal
...
if showMovies
    ...
    for t= 1:nframes;
        hold off
        drawHeadEnrichment(meanHeadAll)
        drawHeadEnrichment(alignedAll(:,:,t)); axis square; axis([-70 70 -70 70])
        drawnow
    end
end
...
if showMovies
    ...
    for t = 1:size(pts,3);
        ...
        drawHeadEnrichment(currentHead,1);
