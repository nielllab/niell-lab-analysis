%%% makes videos of prey capture from compiled data
%%% mouse/cricket locations, eye position, and parameters that define approaches
%%% cmn 08-2019

close all
framerate =30;

[f p] = uigetfile('*.mat','compiled file');
load(fullfile(p,f));
...
for m = 38:165%length(useData);
    ...
    for i = (1+startLag):length(Rth)-1
        ...
        eyeOffset = 20;
        rth = thetaHead{vid}(i) + eyeOffset + Rth(i);
        lth = thetaHead{vid}(i) - eyeOffset + Lth(i);
        ...
        axis equal; axis([0 1800 0 1200]);  hold off
    ...
    movObj = VideoWriter(fullfile(savepath,f));
    open(movObj);
