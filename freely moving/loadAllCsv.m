...
for j=1:length(fileList) %%% loop over all top camera files
    ...
    if deInter
        Rfname = strrep(Rfname,'DeepCut','_DeInterDeepCut');
    end
    ...
    if savePDF
        ...
        filen=sprintf('%s%s%s%s',ani,date,sessionnum,clipnum,'cmnTest_deInter','.pdf')
...
pFile='T:\PreyCaptureAnalysis\Data\';
 afilename=sprintf('%s',ani,'_cmnTest_deInter','.mat')
