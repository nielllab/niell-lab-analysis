    ...
    for t=1:2
       ...
       % ylim ([-5 20]);xlim([-5 20]);
        ...
        plot(peakresp(inhAll==1  &used,mv,1),...
            peakresp(inhAll==1 & used,mv,2),'ko','Markersize',15,'linewidth',2,'MarkerFaceColor',[.9 .2 .5]);
...
if mv==1
  set(gcf,'Name','stationary');
    ...
    for t=1:2
        ...
        posterr=shadedErrorBar(1:length(mnpost),mnpost,sempost,'r',1); xlim([0 300]);
        ylim([0 1.1]);
  ...
  set(gcf,'Name','mv');
    ...
    for t=1:2
        ...
        for s= 1: length(use)
            ...
            %normalize response of all cells used, then average
            prerange = (mnTetDrift(use(s),:,mv,1))
            normrange(s,:)= max(prerange)%-min(prerange)
            prenorm(s,:) =(mnTetDrift(use(s),:,mv,1)-min(mnTetDrift(use(s),:,mv,1)))... %
                ./normrange(s,1);
            
            postnorm(s,:) =(mnTetDrift(use(s),:,mv,2)-min(mnTetDrift(use(s),:,mv,2)))...
                ./normrange(s,1);
        ...
        subplot(1,2,t)
        preerr=shadedErrorBar(1:length(mnpre),mnpre,sempre,'b',1); hold on; axis square
        posterr=shadedErrorBar(1:length(mnpost),mnpost,sempost,'r',1); xlim([0 140]);
        ylim([0 1.1]);
        set(gca,'FontSize',24);set(gca, 'linewidth', 2)
