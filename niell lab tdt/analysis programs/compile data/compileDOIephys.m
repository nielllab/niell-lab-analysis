...
%use =  find( strcmp({files.treatment},'DOI') & strcmp({files.notes},'good data'))
...
%     drawnow    
    ...
    if ~isempty(files(use(i)).blockWn{1}) & ~isempty(files(use(i)).blockWn{2})
        dt = 1;
        [preCorr postCorr cv2 R eigs] =  prepostDOIdarkness(clustfile,afile,files(use(i)).blockWn,dt,1);
    ...
    if ~isempty(files(use(i)).blockDrift{1}) & ~isempty(files(use(i)).blockDrift{2})
        dt = 1;
        [preCorr postCorr cv2 R eigs ] =  prepostDOIdarkness(clustfile,afile,files(use(i)).blockDrift,dt,1);
    ...
    if ~isempty(files(use(i)).blockDark{1}) & ~isempty(files(use(i)).blockDark{2})
        hasDark(cellrange)=1;
        dt = 1;
        [preCorr postCorr cv2 R eigs] = prepostDOIdarkness(clustfile,afile,files(use(i)).blockDark,dt,1);
...
for mv =1
    ...
    for t=1:2
        ...
        plot(peakresp(inhAll==0 & used,mv,1),...
            peakresp(inhAll==0 & used,mv,2),'.','Markersize',20);
        hold on;
        set(gca,'FontSize',18);
       ...
       % ylim ([-8.5 24]);xlim([-8.5 24]);
        mpre=nanmean(peakresp(inhAll==1 & used,mv,1))
        mpost=nanmean(peakresp(inhAll==1 & used,mv,2))
...
for mv = 1:2
    for t=1:2
        preR = mean(drift_orient(goodAll==1&treatment==t& hasDrift==1 & ~inhAll,:,mv,1),2);
        postR = mean(drift_orient(goodAll==1  &treatment==t& hasDrift==1& ~inhAll,:,mv,2),2);
        deltaR = postR-preR;
...
for t=1:2
   ...
   %xlim([-1 25]); ylim([-1 25]);
    ...
    pre = amp_wn(usedAll,mv,1); post = amp_wn(usedAll,mv,2);
    c = corrcoef(pre,post,'rows','pairwise');c = c(2,1);
    mdl_wnEv= fitlm(amp_wn(usedAll,mv,1),amp_wn(usedAll,mv,2))
    rsquared_wnEv(t) = mdl_wnEv.Rsquared.Ordinary
    n_cells(t) = sum(usedAll)
    text(-20, 33, ['n = ' num2str(n_cells(t))],'FontSize',20)
%     text(-20, 28, ['r^2 = ' num2str(rsquared_wnEv(t),'%.2f')],'FontSize',20)
%     text(-20, 23, ['c.c = ' num2str(c,'%.2f')],'FontSize',20)
...
for mv = 1:2
    ...
    for t=1:2
        subplot(1,2,t)
        used = goodAll==1 & treatment==t & hasWn==1;
        plot(evoked(used & inhAll==0,mv,1),evoked(used & inhAll==0,mv,2),'.','MarkerSize',20);
        title(titles{t});
        ...
        plot(evoked(used & inhAll==1,mv,1),evoked(used & inhAll==1,mv,2),'.r','MarkerSize',20); axis square;
...
%% spont calculcated from cyc avg, mean of running/stationary
...
figure
for i = 1:2
    h= hist(wnCorr(corrTreatment==i,1),-1:.1:1);
    h2=hist(wnCorr(corrTreatment==i,2),-1:.1:1);
    subplot(1,2,i);
...
for i = 1:2
    ...
    plot(h,'-b');hold on;
    plot(h2,'-r')
    plot([-0.5 1],[-0.5 1]); axis([-0.5 1 -0.5 1]);
    %xlim([-1 1]);
    title(titles{i});
    legend('Pre-drug','Post-drug')
...
%% pref psth - normalized to pre, replace run/stat

 peakresp = squeeze(max(drift_orient,[],2))-squeeze(drift_spont);%subtract driftspont
% peakresp = squeeze(mean(drift_orient,2))-squeeze(drift_spont);%subtract driftspont
amp=peakresp;
useResp = amp(:,1,1)>1 | amp(:,1,2)>1;
...
figure
for t=1:2
    if i==1, set(gcf,'Name','pref ori grating lyr5 mv');
        used =data& layerAll==5 & treatment==t;
        mn = squeeze(nanmean(tcourse_pref(used,1,:,:),1))';
        ...
        sem = nanstd(tcourse_pref(used,1,:,:),1)/sqrt(sum(used));
        sem=squeeze(sem);sem=sem';
        ...
        ncells = sum(used);
    elseif i ==2,set(gcf,'Name','pref ori grating lyr 2/3 mv');
        used =data& (layerAll==2|layerAll==3) & treatment==t;
        mn = squeeze(nanmean(tcourse_pref(used,1,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
        mnpost = (mn(:,2)-min(mn(:,2)))/range; mnpost = circshift(mnpost,10);
        sem = nanstd(tcourse_pref(used,1,:,:),1)/sqrt(sum(used));sem=squeeze(sem);sem=sem';
        semrange1 = max(sem(:,1)) - min(sem(:,1)); semrange2 = max(sem(:,2)) - min(sem(:,2));
        sempre = (sem(:,1) - min(sem(:,1))) / semrange1; sempre = circshift(sempre,10); 
        sempost = (sem(:,2) - min(sem(:,2))) / semrange2; sempost = circshift(sempost,10);
        ncells = sum(used);
    elseif i ==3, set(gcf,'Name','pref ori grating inh mv');
        used =dataInh& treatment==t;
        mn = squeeze(nanmean(tcourse_pref(used,1,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
...
for i =1:3
    clear range mnpre mnpost sempre sempost posterr preerr

    figure
    for t=1:2
        if i ==1
            used = data& layerAll==5 & treatment==t;
            range = max(tcourse_pref(used,1,1,:),[],4)
            A = squeeze(bsxfun(@minus, tcourse_pref(used,1,1,:), min(tcourse_pref(used,1,1,:),[],4)));
            B = squeeze(bsxfun(@minus, tcourse_pref(used,1,2,:), min(tcourse_pref(used,1,2,:),[],4)));

            prenorm = bsxfun(@rdivide,A,range);
            postnorm = bsxfun(@rdivide, B,range);
            
            %    postnorm = tcourse_pref(used,1,2,:)- min(tcourse_pref(used,1,2,:),1)./range;
            mnpre = nanmean(prenorm,1); mnpre=squeeze(mnpre)'; mnpre = circshift(mnpre,[0 10]);
            mnpost = nanmean(postnorm,1); mnpost=squeeze(mnpost)'; mnpost = circshift(mnpost,[0 10]);
            sempre = nanstd(prenorm)/sqrt(sum(used));sempre=squeeze(sempre)';sempre = circshift(sempre,[0 10]);
            sempost = nanstd(postnorm)/sqrt(sum(used));sempost=squeeze(sempost)'; sempost = circshift(sempost,[0 10]);
            ncells = sum(used);
        ...
        elseif i ==3, set(gcf,'Name','pref ori grating inh mv');
            used =dataInh& treatment==t;
            range = max(tcourse_pref(used,1,1,:),1)% - min(tcourse_pref(used,1,1,:),1); %set range w/pre data to normalize to
            range = max(tcourse_pref(used,1,1,:),[],4)
            A = squeeze(bsxfun(@minus, tcourse_pref(used,1,1,:), min(tcourse_pref(used,1,1,:),[],4)));
            B = squeeze(bsxfun(@minus, tcourse_pref(used,1,2,:), min(tcourse_pref(used,1,2,:),[],4)));
        ...
        subplot(1,2,t)
        x = ((1:length(mnpre)-5)*dt -dt/2);
        preerr=shadedErrorBar(x,mnpre(1:45),sempre(1:45)','b',1); hold on; axis square
        posterr=shadedErrorBar(x,mnpost(1:45),sempost(1:45)','r',1);
        axis square; set(gca,'fontsize', 18); hold on; xlim([0 2.5])
        title(titles{t}); xlabel('time (s)'); ylabel('spikes/sec');
        text(1.75,max(max((mnpre))+.75), ['n = ' num2str(ncells)],'FontSize',20);
...
%% mean of run/stat pref theta
clear useResp useOsi OsiMn ampmean
OsiMn = squeeze(mean(drift_osi,2));
useOsi= (OsiMn(:,1)>.3 |OsiMn(:,2)>.3)';
...
drift_pref_theta_mn = squeeze(mean(drift_pref_theta,2));
data = goodAll & useResp' & ~inhAll & hasDrift==1 & useOsi==1 ;%which cells to include
dataInh =goodAll & useResp' & inhAll & hasDrift==1 & useOsi==1;
dataAll =goodAll & useResp' & hasDrift==1 &useOsi==1;
titles={'Saline', 'DOI'};
figure
set(gcf,'Name', 'mean of running and stat, osi.3');
for t=1:2
    pre=drift_pref_theta_mn(dataAll&treatment==t,1); post= drift_pref_theta_mn(dataAll&treatment==t,2)
    post(pre-post > pi/2) = post(pre-post>pi/2)+pi; pre(post-pre > pi/2) = pre(post-pre>pi/2)+pi;
        ...
        rs_pf(t) = mdl_pf.Rsquared.Ordinary
        subplot(1,2,t)
        pre=drift_pref_theta_mn(data&treatment==t,1); post= drift_pref_theta_mn(data&treatment==t,2)
        post(pre-post > pi/2) = post(pre-post>pi/2)+pi; 
        pre(post-pre > pi/2) = pre(post-pre>pi/2)+pi;
        plot(pre,post,'.','Markersize',18)
        hold on; axis square;plot([0 3.25], [0 3.25]);xlim([0 3.25]);ylim([0 3.25]);
        
        pre=drift_pref_theta_mn(dataInh&treatment==t,1); post= drift_pref_theta_mn(dataInh&treatment==t,2)
        post(pre-post > pi/2) = post(pre-post>pi/2)+pi; 
        pre(post-pre > pi/2) = pre(post-pre>pi/2)+pi;
        plot(pre,post,'r.','Markersize',18);title(titles{t});   
       %text(.2, 2.8, ['r^2 = ' num2str(rs_pf(t),'%.2f')],'FontSize',18)
...
amp=peakresp;
useResp = amp(:,1,1)>3 | amp(:,1,2)>3;% |amp(:,2,1)>2 | amp(:,2,2)>2;
data = goodAll & useResp' & inhAll==0 & hasDrift==1 ;%which cells to include
dataInh =goodAll & useResp' & inhAll==1 & hasDrift==1 ;
dataAll =goodAll & useResp' & hasDrift==1 ;
...
%% mnpsth stationary - normalized to pre

for i=1:3
    clear used
figure
for t=1:2
    if i==1, set(gcf,'Name','grating lyr5 stationary');
        used =data& layerAll==5 & treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,1,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
        mnpost = (mn(:,2) - min(mn(:,2)))/range; mnpost = circshift(mnpost,10);
        sem = std(mnPsth_mv(used,:,:),1)/sqrt(sum(used));
        sem=squeeze(sem);sem=sem';
        semrange1 = max(sem(:,1)) - min(sem(:,1)); semrange2 = max(sem(:,2)) - min(sem(:,2));
        sempre = (sem(:,1) - min(sem(:,1))) / semrange1; sempre = circshift(sempre,10); 
        sempost = (sem(:,2) - min(sem(:,2))) / semrange2; sempost = circshift(sempost,10);
        ncells = sum(used);
    elseif i ==2,set(gcf,'Name','grating lyr 2/3 stat');
        used =data& (layerAll==2|layerAll==3) & treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,1,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
        mnpost = (mn(:,2)-min(mn(:,2)))/range; mnpost = circshift(mnpost,10);
        sem = std(mnPsth_mv(used,:,:),1)/sqrt(sum(used));sem=squeeze(sem);sem=sem';
        semrange1 = max(sem(:,1)) - min(sem(:,1)); semrange2 = max(sem(:,2)) - min(sem(:,2));
        sempre = (sem(:,1) - min(sem(:,1))) / semrange1; sempre = circshift(sempre,10); 
        sempost = (sem(:,2) - min(sem(:,2))) / semrange2; sempost = circshift(sempost,10);
        ncells = sum(used);
    elseif i ==3, set(gcf,'Name','grating inh stat');
        used =dataInh& treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,1,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
...
%% mnpsth mv - normalized to pre


amp=peakresp;
useResp = amp(:,2,1)>2 | amp(:,2,2)>2;% |amp(:,2,1)>2 | amp(:,2,2)>2;
data = goodAll & useResp' & inhAll==0 & hasDrift==1 ;%which cells to include
dataInh =goodAll & useResp' & inhAll==1 & hasDrift==1 ;
dataAll =goodAll & useResp' & hasDrift==1 ;
...
figure
for t=1:2
    if i==1, set(gcf,'Name','grating lyr5 mv');
        used =data& layerAll==5 & treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,2,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
        mnpost = (mn(:,2) - min(mn(:,2)))/range; mnpost = circshift(mnpost,10);
        sem = std(mnPsth_mv(used,:,:),1)/sqrt(sum(used));
        ...
        sempost = (sem(:,2) - min(sem(:,2))) / semrange2; sempost = circshift(sempost,10);
        ncells = sum(used);
    elseif i ==2,set(gcf,'Name','grating lyr 2/3 mv');
        used =data& (layerAll==2|layerAll==3) & treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,2,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
        ...
        sempost = (sem(:,2) - min(sem(:,2))) / semrange2; sempost = circshift(sempost,10);
        ncells = sum(used);
    elseif i ==3, set(gcf,'Name','grating inh mv');
        used =dataInh& treatment==t;
        mn = squeeze(mean(mnPsth_mv(used,2,:,:),1))';
        range = max(mn(:,1)) - min(mn(:,1)); %set range w/pre data to normalize to
        mnpre = (mn(:,1) - min(mn(:,1))) / range; mnpre = circshift(mnpre,10);
...
for mv=2
     figure
%     if mv ==1,set(gcf,'Name','stationary mean OSI')
%         useResp = peakresp(:,1,1)>2 | peakresp(:,1,2)>2;
%         useOsi = drift_osi(:,1,1)>0 | drift_osi(:,1,2)>0
%     else set(gcf,'Name','mv mean OSI')
%         useResp = peakresp(:,2,1)>2 | peakresp(:,2,2)>2;
%         useOsi = drift_osi(:,2,1)>0 | drift_osi(:,2,2)>0
%     end
for t=1:2
%     dataAll = goodAll & useResp' & hasDrift==1 & (useOsi ==1)' &treatment==t;
...
%     if mv ==1,set(gcf,'Name','stationary mean OSI')
%         useResp = peakresp(:,1,1)>2 | peakresp(:,1,2)>2;
%         useOsi = drift_osi_mn(:,1,1)>0 | drift_osi_mn(:,1,2)>0
%     else set(gcf,'Name','mv mean OSI')
%         useResp = peakresp(:,2,1)>2 | peakresp(:,2,2)>2;
%         useOsi = drift_osi_mn(:,2,1)>0 | drift_osi_mn(:,2,2)>0
%     end
for t=1:2
%     dataAll = goodAll & useResp' & hasDrift==1 & (useOsi ==1)' &treatment==t;
