...
for dataset = 1:2  %%%  EO3 vs EO7
    ...
    elseif dataset ==2
        afiles = {'Good recordings\EO7_EO9\05_10_13_EO7\Record1_upper\analysis_1.mat',...
              ...
              'Good recordings\EO7_EO9\11_27_13_EO7\rec2\analysis_11_27_13_EO7_rec2.mat'}; %%% tg
...
figure
plot(alldata(:,5),alldata(:,6),'ro') %all waveforms included in the analysis are clustered here
axis([0 5 0 5]);
...
hold on
plot(alldata(find(broad),5),alldata(find(broad),6),'go');
...
figure
hold on
plot(wvform(find(~inh),:)','g');plot(wvform(find(inh),:)','r');
ylabel('amplitude')
xlabel('msec')
% 
% figure
...
hold on
plot(wvform(age==2 & inh',:)','b');%age==2 = EO3 and age==1 = EO7 plots all inhibotry waveforms as a function of age
