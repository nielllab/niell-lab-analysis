...
%use =  find( strcmp({files.treatment},'DOI') & strcmp({files.notes},'good data')  & ~cellfun(@isempty,{files.predark})& ~cellfun(@isempty,{files.postdark}) & strcmp({files.expt},'042016'))
...
%     if ~isempty(files(i).prepinpFile)
%         pinp =getPinp(clustfile,afile, files(i).prepinpFile,3,5,1,1,1)
...
%     end
%     
%     if ~isempty(files(i).postpinpFile)
%         pinp =getPinp(clustfile,afile, files(i).postpinpFile,3,5,1,1,1)
...
for t = 1:2
    ...
    for i = 1:ceil(length(useN))
        ...
        if inhAll(useN(i)==1) , xlabel('inh'); else  xlabel('exc'); end
    end
...
lowsf= find([drift_sf_trial{:}]==2)
 highsf=find([drift_sf_trial{:}]==6)
...
%        % {cellrange(c),prepost}
%         
%        find([drift_sf_trial{2}]==2)
%        lowsf_psth_pre = {lowsf, drift_trial_psth{cellrange,1}};
%        lowsf_psth_post = {lowsf, drift_trial_psth{cellrange,2}};
% 
%        psth_test_pre = cellfun(@mean, lowsf_psth_pre)
%  
%        
