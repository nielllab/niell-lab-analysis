...
[fWT,xWT] = ecdf(data((wt &lyr==4|lyr==3 & pinped  & used)));

data_WT=data((wt  & pinped  & used));
data_wt_p = nanmedian(data(wt &lyr==4|lyr==3 & pinped  & used));
data_wt_psem=semedian(data(wt &lyr==4|lyr==3 &  pinped  & used));

% data_ko= nanmedian(data(ko1 & lyr&  ~pinped & ~inh & used));
% data_ko_sem=semedian(data(ko1 & lyr& ~pinped & ~inh & used));
[fN2A,xN2A] = ecdf(data((ko1 &lyr==4|lyr==3  & pinped  & used)))

data_N2A=data((ko1 &lyr==4|lyr==3  & pinped  & used));
data_ko_p = nanmedian(data(ko1 &lyr==4|lyr==3& pinped  & used));
data_ko_psem=semedian(data(ko1 &lyr==4|lyr==3 & pinped  & used));

% data_ko2= nanmedian(data(ko2 & lyr&  ~pinped & ~inh & used));
% data_ko2_sem=semedian(data(ko2 & lyr& ~pinped & ~inh & used));
[fN2B,xN2B] = ecdf(data((ko2 &lyr==4|lyr==3  & pinped  & used)))

data_N2B=data((ko2  & pinped  & used))
data_ko_p2 = nanmedian(data(ko2 &lyr==4|lyr==3 & pinped  & used));
data_ko2_psem=semedian(data(ko2 &lyr==4|lyr==3 & pinped  & used));
