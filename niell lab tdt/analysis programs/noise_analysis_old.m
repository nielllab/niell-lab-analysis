...
for cell_n = 1:cell_range
    ...
    channel_times =spikeT{cell_n} - (b-1)*10^5;
    times = channel_times(channel_times>0 & channel_times<10^5);
     frame_duration = median(diff(frameEpocs{b}(2,:)))
    else
        clust_no = [];
        channel_no = cell_n;
    ...
    for f = 1:n_frames
        if SU
            [Spike_Timing index numtrials] = getTrials(frameEpocs{b},times, f, frame_duration);
        else
