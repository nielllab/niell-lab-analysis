function drift_analysis_mv(clustfile,afile,pdfFile,Block_Name,blocknum)
...
for mv_state = 1:2;
    ...
    for rep=extra_range
        ...
        if SU
            [Spike_Timing index numtrials] = getTrialsSU_mv(epocs,times, cond, stim_duration,tsamp,vsmooth, mv_state,0.5);
        else
            [Spike_Timing index numtrials] = getTrialsSU_mv(epocs,data.MUspikeT{cell_n}, cond, stim_duration,tsamp,vsmooth,mv_state,0.5);
        end
        ...
        if rep==1
            xlabel('spont')
            for f = 0:2
                drift_mv(cell_n,mv_state).spont(f+1) = spikeFFT(spikes,tf(1)*f)/(stim_duration*numtrials);
            end
        else
            for f = 0:2  %%% fix this when adding more tf for flicker
                drift_mv(cell_n,mv_state).flicker(rep-1,f+1) = spikeFFT(spikes,tf(1)*f)/(stim_duration*numtrials);
            end
            xlabel('flicker')
        end
    end
    ...
    for rep =1:panels
        ...
        for c =1:nrows*ncols;
           ...
           if SU
            ...
            spikes=spikes(spikes>latency & spikes<stim_duration+latency);
            for f = 0:2
                drift_mv(cell_n,mv_state).R(orient_ind,sf_ind,rep,f+1) = spikeFFT(spikes,tf(rep)*f)/(stim_duration*numtrials);
            end
            ...
            hold on;
           % plot([0 max(hist_range)], [drift_mv(cell_n,mv_state).spont(1) drift_mv(cell_n,mv_state).spont(1)],'g')
            axis(axis_range);
            set(gca,'XTickLabel',[])
            set(gca,'YTickLabel',[])
        ...
        for f = 1:3
            ...
            imagesc(squeeze(abs(drift_mv(cell_n,mv_state).R(:,:,rep,f))));
            colorbar
        end
    ...
    color={'b','r'};
    drift_mv(cell_n,mv_state).orient_tune = zeros(3,length(tf),nrows);
    drift_mv(cell_n,mv_state).sf_tune=zeros(3,length(tf),ncols+1);
    ...
    for f=1:3
        for rep = 1:length(tf)
            ...
            [u s v] = svd(abs(squeeze(drift_mv(cell_n,mv_state).R(:,:,rep,f)))-abs(drift_mv(cell_n,mv_state).spont(f)))
            %[u s v] = svd(abs(squeeze(drift_mv(cell_n,mv_state).R(:,:,rep,f))))
            ...
            if rep<=(length(extra_range)-1)
                sf_tune(1) = abs(drift_mv(cell_n,mv_state).flicker(rep,f));  %%% fix this when adding more tfs for flicker
            else
                sf_tune(1) = abs(drift_mv(cell_n,mv_state).flicker(length(extra_range)-1,f));
            end
            drift_mv(cell_n,mv_state).orient_tune(rep,f,:)=orient_tune';
            drift_mv(cell_n,mv_state).sf_tune(rep,f,:) = sf_tune;
    ...
    print('-dpsc',psfilename,'-append');
      ...
      alltune = squeeze(drift_mv(cell_n,mv_state).R(:,:,1,1));  
    figure
    hold on
    cols = 'bgrcmyk';
