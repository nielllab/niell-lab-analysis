...
maxLength=600;
startT=TTX.CurBlockStartTime;
endT = TTX.CurBlockStopTime;
duration = endT-startT;

nchunks = ceil(duration/maxLength);
W = [];
t = [];

for c = 1:nchunks
    sprintf('reading chunk %d of %d',c,nchunks)
    TTX.SetGlobalV('WavesMemLimit',2.1e9);
    TTX.SetGlobalV('MaxReturn',1e7);
    TTX.SetGlobalV('Channel', 0);
    TTX.SetGlobals('FillItem=DataPoints')
    TTX.SetGlobalV('T2', min(duration,c*maxLength));
    TTX.SetGlobalV('T1', (c-1)*maxLength);
    
    % event_code
    tic
    W= [W  TTX.ReadWavesV(event_code)'];
    toc
    size(W)
