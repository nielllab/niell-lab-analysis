function moviedata=generateNoise_xyt(maxSpatFreq,maxTempFreq,contrastSigma,duration,movtype,binarize,imsize, nimg, mag);
...
%%% movietype 0 = standard noise (no modulation)
...
%%% other params
%%% imsize = 256            %%% size in pixels
%%% nimg = 4;               %%% # of images to save out
%%% mag  = 4;               %%% amount to zoom the image (for smooth pixels)
...
[bname output_path] = uiputfile('*.tif','image names');
for f = 1:nimg
    fnum = floor(rand*size(moviedata,3));
    fname = fullfile(output_path,sprintf('%s_%d.tif',bname(1:end-4),f));
    imwrite(imresize(moviedata(:,:,fnum),mag),fname)
end
