...
for cond = 1:6
    ...
    data(cond,1) = nanmean(freqApproach(trials)');
    err(cond,1) = nanstd(freqApproach(trials)')/sqrt(length(trials));
    data(cond,2) = nanmean(approachSuccess(trials));
    err(cond,2) = nanstd(approachSuccess(trials))/sqrt(length(trials));
    
    ContactData(cond,1)=nanmean(firstContact(trials)');
    Contacterr(cond,1) = nanstd(firstContact(trials)')/sqrt(length(trials));
    
    ContactData(cond,2)=nanmean(interContact(trials)');
    Contacterr(cond,2) = nanstd(interContact(trials)')/sqrt(length(trials));
    
    ContactData(cond,3)=nanmean(nContacts(trials)');
    Contacterr(cond,3) = nanstd(nContacts(trials)')/sqrt(length(trials));
    
    ContactData(cond,4)=nanmean(contactDuration(trials)');
    Contacterr(cond,4) = nanstd(contactDuration(trials)')/sqrt(length(trials));
    
    ContactData(cond,5)=nanmean(captureProb(trials)');
    ...
    for tr = 1:length(trials);
        ...
        for i = 1:length(AppR{trials(tr)});
            ...
            [val start] =min(abs(abs(y)-90));%find first point where prey azimuth is <90  %%% replaced function nearest, unknown?
            if ~isempty (preTouch);
                dist=preTouch(1);
            else
                mindist=min(x);
    ...
    preContT(cond,1)=nanmean(AppErr);
...
figure
bar(data(1:4,1)); hold on; errorbar(1:4,data(1:4,1),err(1:4,1),'o')
ylabel('approach frequency')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});


figure
bar(data(1:4,2)); hold on; errorbar(1:4,data(1:4,2),err(1:4,2),'o')
ylabel('prey intercept')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});
title('figure 3D bottom')

figure
bar(ContactData(1:4,1)); hold on; errorbar(1:4,ContactData(1:4,1),Contacterr(1:4,1),'o')
ylabel('FirstContactTime')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});
title('figure 3E top')

figure
bar(ContactData(1:4,2)); hold on; errorbar(1:4,ContactData(1:4,2),Contacterr(1:4,2),'o')
ylabel('InterContactInterval')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});
title('figure 3E bottom')

figure
bar(ContactData(1:4,4)); hold on; errorbar(1:4,ContactData(1:4,4),Contacterr(1:4,4),'o')
ylabel('Contact duration')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});
title('fig3')

figure
bar(ContactData(1:4,5)); hold on; errorbar(1:4,ContactData(1:4,5),Contacterr(1:4,5),'o')
...
figure
plot(thetabins,nanmean(PreyE_mid{2},2),'k','LineWidth',3); hold on
plot(thetabins,nanmean(PreyE_mid{1},2),'b','LineWidth',3);
plot(thetabins,nanmean(PreyE_mid{3},2),'g','LineWidth',3);
plot(thetabins,nanmean(PreyE_mid{4},2),'r','LineWidth',3);

title 'Prey eccentricity all conditions >5cm mouse moving'

figure
plot(thetabins,nanmean(PreyE_mid{1},2),'b','LineWidth',3); hold on%eye suture light
plot(thetabins,nanmean(PreyE_mid{2},2),'k','LineWidth',3);%eye suture light
plot(thetabins,nanmean(PreyE_mid{5},2),'c','LineWidth',3); %eye suture light
plot(thetabins,nanmean(PreyE_mid{6},2),'m','LineWidth',3); %eye suture


% PLOT ALL CONDITIONS range over trial
figure
plot(nanmean(Rsession{2},2),'k','LineWidth',3);hold on
plot(nanmean(Rsession{1},2),'b','LineWidth',3);hold on
plot(nanmean(Rsession{5},2),'c','LineWidth',3);hold on
plot(nanmean(Rsession{6},2),'m','LineWidth',3);
