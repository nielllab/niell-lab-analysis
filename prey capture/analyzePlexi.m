function [latency, tracks, mouseTouch, cricketEdge,mouseTouchEd,MouseTouchL, Rstarts, hits, range, tdhist, cricketTouch] = analyzePlexi(fname,fps,scale)
...
%     th{i} = mod(thetaR{i}+180,360)-180
    ...
    % where does the mouse touch
        if head{i}(end,1) < wallL(1) +15;
            MouseTouchLeft=1;
        elseif head{i}(end,1) > wallR(1)-15;% touch point of track is within 10cm of right wall
            MouseTouchLeft=0;
        end
...
%     title 'dist to targ by diff-Y to Target'
  ...
  % exclude wall touch where cricket was in corner
    ...
    if dist>5 & hit==1 & cricketCorner==0
