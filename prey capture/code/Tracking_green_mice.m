...
%pathname='E:\Dropbox\mucsimol_V1\Trial videos for Tristans tracking\Iphone5\live_2_LT_1hr_postCNO'
%pathname = 'W:\GreenEarTracking\iPhone\5_25_17\ALGS0101\'
cd 'F:\GreenEarTracking\iPhone\5_25_17\'

%pathname='F:\GreenEarTracking\iPhone\5_25_17\ALGSo101_PS\'
pathname='F:\GreenEarTracking\iPhone\5_25_17\BMQM4759\'
...
%trim(:,:,:,:)=img(20:1041,400:1700,:,:);

% for i=1:600%size(img,4)%
%    imshow(img(:,:,:,i));
%   % imshow(trim(:,:,:,500));
%    drawnow   
% end
...
% 
% for i=1:500%length(files)  
%  imshow(trim(:,:,:,i));
%   drawnow
% end

%imtool (trim(:,:,:,184))
scale=30; %30 pixels/cm
fps=30; % could change between videos

%% insert function for color thresholding
%%
numframes =600% size(img, 4); determine number of frames containing 3 approaches for virtual trial
...
type=0% type=1 is live type=0 equals virtualt
...
CentroidB = zeros(numframes, 2);
CentroidE1=zeros(numframes, 2);
CentroidE2=zeros(numframes, 2);
CentroidC=zeros(numframes, 2);
AreaC=zeros(numframes, 1);
end

%ID ear and body objects and create tracks
for k = 1:numframes
...
%     AreaC(k)=sC(idx(1)).Area;
    ...
    imshow(img(:,:,:,k));
     [X,Y]=getpts; %1st point left ear 2nd pt right ear 3rd pt body
    CentroidE1(k,:)=[X(1),Y(1)];
    CentroidE2(k,:)=[X(2),Y(2)];
    CentroidB(k,:)=[X(3),Y(3)];
    %CentroidC(k,:)=NaN;
end
end
...
if type==0
    ...
    for k = 1:numframes
        ...
        Lc = bwlabel(mask7(:,:,k)); %mask for the cricket
        sc = regionprops(Lc, 'area', 'centroid','Eccentricity');
        area_vector = [sc.Area];
        E_vec=[sc.Eccentricity];
        big=find(area_vector>45 & E_vec<0.93 );
...
clear Left Right
[Left, Right]=LeftEar(img,CentroidB,CentroidE1,CentroidE2,0); %1 in last entry position implies automatic assignment of Left ear vs. right, if "0" not auto 1st click is left ear, then Shift+ click the right ear center
...
touch= find(DistLE_C<65 |DistB_C<75 );
% j=70
% figure
% imagesc(img(:,:,:,j)); hold on
% plot(CentroidC(j,1),CentroidC(j,2),'c*');hold on

%omit cricket from frames where mouse is touching it or misidentified as
%part of mouse
...
plot(Right(:,1),Right(:,2),'g');hold on
plot(CentroidB(:,1),CentroidB(:,2),'c'); hold on
plot(CentroidC(:,1),CentroidC(:,2),'k*');%% generate movie of tracked points
...
[Tfname Tpname] = uiputfile('*Tracks.mat','Tracks file');
fullTname = fullfile(Tpname, Tfname)
save(fullTname, 'Left','Right','CentroidB', 'CentroidCNT')
