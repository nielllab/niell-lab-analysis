...
for tr =110 %1:length(head); % trials(example)%44 is the start of the newly added Ear plug videos
...
PreyE_mid=thetahist_midMove(:,trials);
figure
plot(thetabins,nanmean(PreyE_mid,2),colorlabel(cond),'LineWidth',3);
title(sprintf('Prey eccentricity whole session %s',Gr{cond}));

%range over whole session
Rsession=rhist(:,trials);
figure
plot(dbins,nanmean(Rsession,2),colorlabel(cond),'LineWidth',3);
...
nAll=sum(n,3);
stdE_L=nanstd(avg_theta_L,[],3)./sqrt(nAll);

figure; hold on
plot(dbin,nanmean(avg_theta_L,3),'b','LineWidth',2);hold on
plot(dbin,(nanmean(avg_theta_L,3)-stdE_L),'k','LineWidth',1.5);
plot(dbin,(nanmean(avg_theta_L,3)+ stdE_L),'k','LineWidth',1.5);
...
for tr = 1:length(trials);
    ...
    for i = 1:length(AppR{trials(tr)});
        ...
        start=nearest(abs(y),90);%find first point where prey azimuth is <90
        if ~isempty (preTouch);
            dist=preTouch(1);
        else
            mindist=min(x);
...
preContT(1,1)=nanmean(AppErr);
preContErr(1,1)=nanstd(AppErr)/sqrt(length(AppErr));
...
plot(binD,nanmean(avg_theta),'b','LineWidth',2);hold on
...
%% to hist distribution to determine natural distance cutoff
clear dbin ebin avg_theta thetaRangeHist e n nAll stdE
dbin=1:2:40;
ebins=0:15:130;

for tr=1:length(trials);
    clear dist T
    dist=rangeSmooth{trials(tr)};%extract the trials range
    Te=abs(thetaSmoothC{trials(tr)});%extract the trial's theta error
    [thetaRangeHist(:,:,tr),avg_theta_D(:,:,tr)]=myHist2Avg(dist,Te,dbin,ebins);
end
...
nAll=sum(n,3);
stdE_D=nanstd(avg_theta_D,[],3)./sqrt(nAll);

figure; hold on
plot(dbin,nanmean(avg_theta_D,3),'k','LineWidth',2);hold on
plot(dbin,(nanmean(avg_theta_D,3)-stdE_D),'k','LineWidth',1.5);
plot(dbin,(nanmean(avg_theta_D,3)+ stdE_D),'k','LineWidth',1.5);
set(gca,'xdir','reverse','XLim',[0 40],'YLim',[0 180])
...
preContT(2,1)=nanmean(AppErr);
preContErr(2,1)=nanstd(AppErr)/sqrt(length(AppErr));
...
n=sum(~isnan(avg_theta));
stdE=nanstd(avg_theta,[],1)./sqrt(n);
plot(binD,nanmean(avg_theta),'k','LineWidth',2);hold on
plot(binD,(nanmean(avg_theta)-stdE),'k','LineWidth',1.5);
plot(binD,(nanmean(avg_theta)+ stdE),'k','LineWidth',1.5);
...
for tr=1:length(trials);
    ...
    dist=rangeSmooth{trials(tr)};%extract the trials range
    Te=abs(thetaSmoothC{trials(tr)});%extract the trial's theta error
    [thetaRangeHist(:,:,tr),avg_theta_EPL(:,:,tr)]=myHist2Avg(dist,Te,dbin,ebins);
end
...
nAll=sum(n,3);
stdE_EPL=nanstd(avg_theta_EPL,[],3)./sqrt(nAll);

figure; hold on
plot(dbin,nanmean(avg_theta_EPL,3),'g','LineWidth',2);hold on
plot(dbin,(nanmean(avg_theta_EPL,3)-stdE_EPL),'k','LineWidth',1.5);
plot(dbin,(nanmean(avg_theta_EPL,3)+ stdE_EPL),'k','LineWidth',1.5);
set(gca,'xdir','reverse','XLim',[0 40],'YLim',[0 180])
...
preContT(3,1)=nanmean(AppErr);
preContErr(3,1)=nanstd(AppErr)/sqrt(length(AppErr));
...
n=sum(~isnan(avg_theta));
stdE=nanstd(avg_theta,[],1)./sqrt(n);
plot(binD,nanmean(avg_theta),'g','LineWidth',2);hold on
plot(binD,(nanmean(avg_theta)-stdE),'k','LineWidth',1);
plot(binD,(nanmean(avg_theta)+ stdE),'k','LineWidth',1);
...
Rsession=rhist(:,trials);
figure
plot(dbins,nanmean(Rsession,2),'r','LineWidth',3);
...
for tr=1:length(trials);
    ...
    dist=rangeSmooth{trials(tr)};%extract the trials range
    Te=abs(thetaSmoothC{trials(tr)});%extract the trial's theta error
    [thetaRangeHist(:,:,tr),avg_theta_EPD(:,:,tr)]=myHist2Avg(dist,Te,dbin,ebins);
end
...
nAll=sum(n,3);
stdE_EPD=nanstd(avg_theta_EPD,[],3)./sqrt(nAll);

figure; hold on
plot(dbin,nanmean(avg_theta_EPD,3),'r','LineWidth',2);hold on
plot(dbin,(nanmean(avg_theta_EPD,3)-stdE_EPD),'k','LineWidth',1.5);
plot(dbin,(nanmean(avg_theta_EPD,3)+ stdE_EPD),'k','LineWidth',1.5);
set(gca,'xdir','reverse','XLim',[0 40],'YLim',[0 180])


%% approach paths for ear plug dark
...
preContT(4,1)=nanmean(AppErr);
preContErr(4,1)=nanstd(AppErr)/sqrt(length(AppErr));
...
n=sum(~isnan(avg_theta));
stdE=nanstd(avg_theta,[],1)/sqrt(n);
plot(binD,nanmean(avg_theta),'r','LineWidth',2);hold on
plot(binD,(nanmean(avg_theta)-stdE),'k','LineWidth',1.5);
plot(binD,(nanmean(avg_theta)+ stdE),'k','LineWidth',1.5);
...
title 'ES light trials'

PreyE_mid_ESL=thetahist_midMove(:,trials);
figure
plot(thetabins,nanmean(PreyE_mid_ESL,2),'k','LineWidth',3);
title 'Prey eccentricity mouse moving middle dist hearing only'
...
% for i=1:length(trials)
%     hold on
%    plot(thetabins,thetahist_midMove(:,trials(15)))
% end
%
% 3 6 8 10 14
clear goodApp AppEcc AppErr
...
preContT(5,1)=nanmean(AppErr);
preContErr(5,1)=nanstd(AppErr)/sqrt(length(AppErr));
...
n=sum(~isnan(avg_theta));
stdE=nanstd(avg_theta,[],1)/sqrt(n);
plot(binD,nanmean(avg_theta),'c','LineWidth',2);hold on
plot(binD,(nanmean(avg_theta)-stdE),'k','LineWidth',1.5);
plot(binD,(nanmean(avg_theta)+ stdE),'k','LineWidth',1.5);
...
title 'ES dark trials'

PreyE_mid_ESD=thetahist_midMove(:,trials);
figure
plot(thetabins,nanmean(PreyE_mid_ESD,2),'m','LineWidth',3);
title 'Prey eccentricity mouse moving middle dist hearing only'
...
preContT(6,1)=nanmean(AppErr);
preContErr(6,1)=nanstd(AppErr)/sqrt(length(AppErr));

figure; hold on

for i=1:length(trials)
    hold on
    plot(thetabins,thetahist_midMove(:,trials(i)))
end
...
figure
bar(preContT(1:4,1)); hold on; errorbar(1:4,preContT(1:4,1),preContErr(1:4,1),'o')
ylabel('Pre Contact theta Error')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});

figure
bar(preContT([1 2 5 6],1)); hold on; errorbar(1:4,preContT([1 2 5 6],1),preContErr([1 2 5 6],1),'o')
ylabel('Pre Contact theta Error eye suture controls')
set(gca,'Xtick',1:4); set(gca,'XtickLabel',{'light','dark','deaf','dark + deaf'});
