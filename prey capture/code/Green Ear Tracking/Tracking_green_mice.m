...
for k=1:numframes
mask4(:,:,k)=(img(:,:,1,k)>(img(:,:,3,k)+30)).*(img(:,:,1,k)<200).*(~mask3(:,:,k)); %R>B+10 & R<150 & ~mask3: is the red parts of mouse
...
%ID location of ears and body in each frame
for k = 1:numframes
...
%% get cricket track
type=1;% type=1 is live type=0 equals virtual, hold shift and click Cricket when image appears
for k = 1:numframes
   if type==1 
...
sc = regionprops(Lc, 'area', 'centroid','Eccentricity');
area_vector = [sc.Area];
E_vec=[sc.Eccentricity];%& E_vec<0.9
big=find(area_vector>150  );
...
save(fullaname, 'Left','Right','CentroidB', 'CentroidCNT','CentroidE1','CentroidE2','touch');
%save('masks.mat','img','mask3','mask4','mask5','-v7.3')
